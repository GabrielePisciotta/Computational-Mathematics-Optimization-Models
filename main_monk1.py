...
def main():
    ...
    ds = "results/Monk1" #<--- cambiamo qui così propaghiamo il dataset
    makedirs(ds, exist_ok=True)

    print("Loading datasets...")
    X_train, Y_train, X_test, Y_test = load_monk_dataset(1)

    print("Building LBFGS model ... ")

    model = NeuralNetwork(LBFGS(m=10, c1=1e-4, c2=0.9, threshold_gradient_norm=0.00001, threshold_loss=0.01),
    ...
    model2 = NeuralNetwork(MGD(learning_rate=0.9,
                               momentum=0.9,
                               threshold_gradient_norm=0.00001,
                               threshold_loss=0.01),
